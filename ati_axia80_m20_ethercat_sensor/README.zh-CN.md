...
| `runtime_diagnostic_sdo_timeout_ms` | `100` | 运行时 SDO 诊断超时时间，单位 ms |
...
`read()` 路径之外读取 `0x2080: Diagnostic Readings` SDO。每次运行时 SDO 会受
`runtime_diagnostic_sdo_timeout_ms` 限制，并通过 `sdo_success`、`sdo_skipped`、
`sdo_failed` 统计累计结果。如果某次 SDO 超时，驱动会等这个请求结束后才会启动下一次
运行时 SDO，不会并发启动多个 SDO。它会报告外部供电电压、应变片温度和 ATI
